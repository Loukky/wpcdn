...
require '/www/wwwroot/loukky.com/wp-content/plugins/wordfence/vendor/autoload.php' ;
...
//$arr = array('ip'=>$ip,'code2'=>$record->country->isoCode,'name'=>$record->country->name);
...
function CID_get_flag( $ip ) {
	...
	//	$code = strtolower( $country['code2'] );
	//	$name = $country['name'];
	//	$code = strtolower( $countrycode2 );
	//	$name = $countryname;
	$code = strtolower( $country['countrycode2'] );
	...
	$output = str_replace( "%IMAGE_BASE%",  'https://cdn.jsdelivr.net/gh/Loukky/wpcdn/themes/show-useragent/flags', $output );
